...
$q=mysqli_query($link, "SELECT * FROM cart WHERE pid='$poid'") or die(mysqli_error());
if(mysqli_num_rows($q)){
	$row=mysqli_fetch_assoc($q);
	$quan=$row['quantity'];
	$quan+=1;
	if(mysqli_query($link,"UPDATE cart set quantity='$quan' WHERE pid='$poid'")){
		//echo 'Product quantity updated!';
		$success_message = '<div class="signupform_content"><h2><font face="bookman">Product quantity updated in cart successfull!</font></h2>';
	}
	else{
		//echo 'Unable to update the cart!';
		$error_message = '<div class="signupform_content"><h2><font face="bookman">Error!</font></h2>';
	}
}
...
$quan = $_POST['quantity'];
	...
	try {
		...
		if(mysqli_query($link,"INSERT INTO cart (uid,pid,pname,quantity,price,fname,oplace,mobile) VALUES ('$user','$poid','$pName',$quan,'$price','$uname_db','$addr','$mbl')")){

		$success_message = '
		<div class="signupform_content"><h2><font face="bookman">Product added to cart successfull!</font></h2>
	...
	<div class="holecontainer" style=" padding-top: 20px; padding: 0 20%">
		<div class="container signupform_content ">
			<div>
				...
				<h2 style="padding-bottom: 20px;">Add to cart</h2>
						...
						<div>
							<form action="" method="POST" class="registration">
								<div class="signup_form" style="    margin-top: 38px;">
									<div>
										<td>
											<input name="mobile" placeholder="Enter your mobile number" required="required" class="email signupbox" type="text" size="30" >
										</td>
									</div>
									<div>
										<td>
											<input name="address" id="password-1" required="required"  placeholder="Enter your address" class="password signupbox " type="text" size="30" >
										</td>
									</div>
									<div>
										<td>
